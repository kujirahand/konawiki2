...
function plugin_vote_convert($params)
{
  ...
  if ($vote_mode == "edit") {
    ...
    $res .= form_input_hidden("vote_id",
            htmlspecialchars($vote_id, ENT_QUOTES));
    $res .= "<textarea cols='50' rows='4' name='csv'>".
            "$body_html</textarea><br>\n";
    if (konawiki_isLogin_write()) {
      $res .= form_input_submit(
        konawiki_lang("Edit"));
    }
    $res .= "</form>";
    return $res;
  ...
  foreach ($rows as $line) {
    ...
    //----------------------------------
    $frm = 
      "<form action='{$action}#{$vote_hash}' method='post'>".
...
function plugin_vote_action($params)
{
  ...
  if ($vote_mode == "write") {
    ...
    $csv = plugin_vote_csv2array(
      plugin_vote_getLog($vote_id, $selection));
...
function plugin_vote_getLog($vote_id, $selection)
{
  ...
  $sql =
    ...
    " AND plug_name='vote' AND plug_key=? LIMIT 1";
  $r = db_get($sql, [$log_id, $vote_id], 'sub');
  $body = "";
  if (isset($r[0]['body'])) {
    $body = $r[0]['body'];
  } else {
...
function plugin_vote_setLog($vote_id, $csv)
{
  ...
  if ($r) {
    $id = $r[0]['id'];
    $sql =
      "UPDATE sublogs SET body=?, mtime=$now".
      "  WHERE id=?";
    db_exec($sql, [$csv, $id], 'sub');
  }
  else {
    $sql =
      "INSERT INTO sublogs".
      ...
      "  VALUES (?,'vote',?,?,$now,$now)";
    db_exec($sql, [$log_id, $vote_id, $csv], 'sub');
  }
  return TRUE;
}
